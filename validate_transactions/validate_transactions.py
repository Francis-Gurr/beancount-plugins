...
from .utils.event_entries import validate_event_entry
from .utils.event_transactions import is_event_transaction, validate_event_transaction
from .utils.link_documents import create_document_entries
from .utils.opening_balance_transactions import (
    is_opening_balance_transaction,
    validate_opening_balance_transaction,
)
from .utils.owed_transactions import is_owed_transaction, validate_owed_transaction
from .utils.payslip_transactions import (
    is_payslip_transaction,
    validate_payslip_transaction,
)
from .utils.receipt_transactions import (
    is_receipt_transaction,
    validate_receipt_transaction,
)
from .utils.transfer_transactions import (
...
def should_skip(entry):
    ...
    is_excluded_transaction = (
        isinstance(entry, core_data.Transaction)
        and "exclude-entry-from-validation" in entry.tags
    )
    return (
        transaction_filename.endswith("transfers.beancount") or is_excluded_transaction
    )
...
def validate_transactions(entries, unused_options_map):
    ...
    for entry in entries:
        ...
        if isinstance(entry, core_data.Event):
            event_ids.append(entry.meta.id)
            errors.extend(validate_event_entry(entry, event_ids))
        ...
        if (
            isinstance(entry, core_data.Custom)
            and entry.type == "initialise_journal_file"
        ):
        ...
        if isinstance(entry, core_data.Transaction):
            ...
            if is_event_transaction(entry):
                event_transactions.append(entry)
    ...
    for entry in event_transactions:
